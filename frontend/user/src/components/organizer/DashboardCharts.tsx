...
export function normalizePieData(data?: CategoryReportItem[]): PieDataItem[] {
...
export function MonthlyBarChart({ data, loading, error }: ChartProps<MonthlyReportItem>) {
...
export function CategoryPieChart({ data, loading, error }: ChartProps<CategoryReportItem>) {
  if (loading) return <Loader message="Loading chart..." />;
  if (error) return <p className="rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700">{error}</p>;
    ...
    <ResponsiveContainer width="100%" height={320}>
      <PieChart>
        <Pie
          ...
          cy="50%"
          outerRadius={110}
          label={({ category_name, event_count, percentage }: PieDataItem) => `${category_name}: ${event_count} (${percentage}%)`}
        >
          {chartData.map((_, i) => (
