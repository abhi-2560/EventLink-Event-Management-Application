...
import { deleteBanner, deleteEventMedia, uploadBanner, uploadEventMedia } from '../../api/organizerApi';
...
export default function EventMediaManager({ event, onChanged }) {
  ...
  async function remove(mediaId, isBanner = false) {
    setLoading(true);
    try {
      if (isBanner) await deleteBanner(event.event_id);
      else await deleteEventMedia(event.event_id, mediaId);
  ...
  return (
    <section className="rounded-2xl border border-border bg-white p-6 shadow-sm">
      ...
      <input ref={bannerInput} className="hidden" type="file" accept="image/jpeg,image/png,image/webp" onChange={(e) => upload(e.target.files, 'IMAGE', true)} />
      <input ref={imageInput} className="hidden" type="file" multiple accept="image/jpeg,image/png,image/webp" onChange={(e) => upload(e.target.files, 'IMAGE')} />
      <input ref={videoInput} className="hidden" type="file" multiple accept="video/mp4,video/webm" onChange={(e) => upload(e.target.files, 'VIDEO')} />
...
function MediaGrid({ label, media, video, loading, onDelete }) {
  return (
    <div>
...
export function validatePendingMedia(media) {
  return {
