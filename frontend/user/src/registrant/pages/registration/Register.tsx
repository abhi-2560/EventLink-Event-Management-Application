...
import ContactForm from '@registrant/components/registration/ContactForm';
import RegistrationSummary from '@registrant/components/registration/RegistrationSummary';
...
import { useRegistration } from '@registrant/context/RegistrationContext';
import { registrationSchema, buildRegistrationDefaults } from '@registrant/schemas/registrationSchema';
import { showError, showSuccess } from '@shared/utils/toast';
import { toApiError } from '@shared/utils/apiError';
import type { CouponValidation } from '@shared/types/api';
...
export default function Register() {
  ...
  const onSubmit = (formData: RegistrationFormValues) => {
    if (!event || formData.seats_booked > event.available_seats) {
      return;
    }
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      setCouponError(toApiError(err).message);
    }
  };
  ...
  if (!event) {
    return <Container className="py-16"><p className="text-sm text-danger">Event not found.</p></Container>;
  }
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-8 grid gap-8 lg:grid-cols-3">
        ...
        <div className="space-y-6">
          <RegistrationSummary
            ...
            totalAmount={couponPreview ? Number(couponPreview.final_amount) : undefined}
          />
