...
import { CATEGORIES, EVENT_TYPES } from '@shared/constants';

import useDebounce from '@registrant/hooks/useDebounce';
import type { EventSearchFilters } from '@registrant/utils/eventSearch';



const EMPTY_FILTERS: Required<EventSearchFilters> = {
  title: '',
  city: '',
  category: '',
  type: '',
...
export function filtersToSearchParams(localFilters: EventSearchFilters): EventSearchFilters {
  const params: EventSearchFilters = {};
...
interface SearchBarProps {
  ...
  onSearch: (_filters: EventSearchFilters) => void;
  compact?: boolean;
}

export default function SearchBar({ filters = EMPTY_FILTERS, onSearch, compact = false }: SearchBarProps) {
  ...
  const handleChange = (field: keyof EventSearchFilters, value: string) => {
