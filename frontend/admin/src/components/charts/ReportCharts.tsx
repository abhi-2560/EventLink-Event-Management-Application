...
type CategoryDatum = { category_name?: string | null; event_count?: string | number | null };
type MonthlyDatum = { month: string; events: string | number; registrations: string | number; revenue: string | number; organizers: string | number };
type PieDatum = { category_name: string; event_count: number; percentage: number };
type ChartProps<T> = { data?: T[]; loading?: boolean; error?: string };

export function normalizePieData(data?: CategoryDatum[]): PieDatum[] {
...
function pieTooltipFormatter(value: unknown, name: unknown, props: { payload?: Partial<PieDatum> } | undefined): [string, string] {
  const count = props?.payload?.event_count ?? value;
  const pct = props?.payload?.percentage ?? 0;
  return [`${String(count)} (${String(pct)}%)`, props?.payload?.category_name ?? String(name)];
}

function pieLegendFormatter(value: unknown, entry: { payload?: Partial<PieDatum> } | undefined) {
  const count = entry?.payload?.event_count ?? 0;
  const pct = entry?.payload?.percentage ?? 0;
  // return `${value}`;
}

export function MonthlyBarChart({ data, loading, error }: ChartProps<MonthlyDatum>) {
...
export function CategoryPieChart({ data, loading, error }: ChartProps<CategoryDatum>) {
  if (loading) return <Loader message="Loading chart..." />;
  if (error) return <p className="text-sm text-danger">{error}</p>;
