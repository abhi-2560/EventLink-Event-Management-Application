  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <div className="space-y-3 rounded-xl border border-border bg-white p-6 shadow-sm text-sm">
        ...
        {Boolean(data.old_value) && <Row label="Old Value" value={<pre className="mt-1 overflow-auto rounded bg-slate-50 p-2 text-xs">{JSON.stringify(data.old_value, null, 2)}</pre>} />}
        {Boolean(data.new_value) && <Row label="New Value" value={<pre className="mt-1 overflow-auto rounded bg-slate-50 p-2 text-xs">{JSON.stringify(data.new_value, null, 2)}</pre>} />}
      </div>
    </div>
  );
}
