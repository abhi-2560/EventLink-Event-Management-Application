...
import type { Category, CategoryPayload, Id } from '../types/admin';
...
export default function Categories() {
  ...
  const updateMut = useMutation({
    mutationFn: ({ id, payload }: { id: Id; payload: CategoryPayload }) => updateCategory(id, payload),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-categories'] });
      setEditing(null);
  ...
  const archiveMut = useMutation({
    mutationFn: (id: Id) => archiveCategory(id),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-categories'] });
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog open={Boolean(archiveTarget)} title="Archive category?" message={`Archive "${archiveTarget?.name}"?`}
        loading={archiveMut.isPending} onConfirm={() => { if (archiveTarget) archiveMut.mutate(archiveTarget.category_id); }} onCancel={() => setArchiveTarget(null)} />
    </div>
  );
}
