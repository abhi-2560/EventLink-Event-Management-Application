...
type CouponForm = { code: string; description?: string; flat_discount: number; expiry_date?: string; is_active?: boolean };
type ModalState = { mode: 'create' } | { mode: 'edit'; coupon: Coupon };

function toPayload(data: CouponForm): CouponPayload {
  return {
...
export default function Coupons() {
  ...
  const saveMut = useMutation({
    mutationFn: (payload: CouponPayload) => (modal?.mode === 'edit'
  ...
  const filtered = useMemo(() => {
    ...
    return ((data || []) as Coupon[]).filter((c) => {
      return (
        !q ||
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog
        ...
        onConfirm={() => { if (deleteTarget) deleteMut.mutate(deleteTarget.coupon_id); }}
...
function PaginationFooter({ page, totalPages, total, onPageChange }: { page: number; totalPages: number; total: number; onPageChange: (page: number) => void }) {
